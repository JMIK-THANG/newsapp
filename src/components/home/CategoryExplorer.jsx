      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="grid gap-3.5 md:grid-cols-2 lg:grid-cols-3">
          {categories.map((category, index) => (
            <article key={category.name} className={`group overflow-hidden rounded-[26px] border border-[#e3e6e1] bg-white ${index === 0 ? "md:col-span-2 lg:col-span-2 lg:grid lg:grid-cols-[1.15fr_.85fr]" : ""}`}>
              <a className={`block overflow-hidden bg-[#e5e9e3] ${index === 0 ? "aspect-[16/10] lg:aspect-auto lg:min-h-[420px]" : "aspect-[16/10]"}`} href={`#${category.name.toLowerCase()}`} tabIndex="-1">
              ...
              <div className={`flex flex-col p-6 md:p-7 ${index === 0 ? "lg:justify-between lg:p-10" : ""}`}>
                <div className="flex items-center justify-between">
                  <span className="rounded-full bg-[#eef1ec] px-3 py-1.5 text-[9px] font-bold tracking-[.12em] text-[#68736d] uppercase">{category.name}</span>
                ...
                <div className={index === 0 ? "mt-12" : "mt-8"}>
