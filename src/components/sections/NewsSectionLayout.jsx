        ...
        <section className="grid gap-x-7 gap-y-0 sm:grid-cols-2 lg:grid-cols-3" aria-label={`Latest ${title}`}>
          {rest.map((story, index) => (
            <article className="group border-b border-[#dcdde0] py-7" key={story.title}>
              ...
              <h2 className="m-0 text-xl leading-[1.25] font-semibold tracking-[-.02em] text-[#111318]"><Link className="transition hover:opacity-60" to={`/${title.toLowerCase()}/story-${index + 1}`}>{story.title}</Link></h2>
