...
export default function FeatureArticlePage() {
  ...
  return <main className="bg-[#f1eee8] px-3 py-8 md:px-6 md:py-12"><article className="mx-auto max-w-[1280px]">
    ...
    <header className="grid gap-8 border-y border-[#182536] py-8 lg:grid-cols-[1fr_320px] lg:items-end"><div><p className="mb-4 text-[10px] font-bold tracking-[.18em] text-[#4f9488] uppercase">The Chinlung Review</p><h1 className="m-0 max-w-[950px] font-serif text-[clamp(34px,4.8vw,52px)] leading-[1.08] tracking-[-.035em] text-[#0c0c0c]">{article.title}</h1></div><div className="border-t border-[#c8c6c0] pt-5 lg:border-t-0 lg:border-l lg:pl-7"><p className="m-0 text-[16px] leading-7 text-[#4f5359]">{article.summary}</p><p className="mt-5 text-xs">By <strong>{article.author || "Chinlung Today"}</strong><br />{article.date} · {article.readTime || article.time || "5 min read"}</p><ShareStoryButton story={article} /></div></header>
