...
const routeFilter = () => {
  const segment = window.location.hash.split("/")[2];
  if (!segment) return "All News";
  return filters.find((filter) => filter.toLowerCase().startsWith(segment)) || "All News";
};
      ...
      <div className="mx-auto max-w-[1380px]">
        <header className="border-b border-[#dcdde0] pb-7">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="m-0 font-serif text-[clamp(38px,5vw,64px)] leading-none tracking-[-.04em] text-[#111318]">Latest News</h1>
        ...
        <div className="flex gap-6 overflow-x-auto border-b border-[#dcdde0] py-1" aria-label="News categories">
        ...
        <div className="grid gap-12 pt-8 lg:grid-cols-[minmax(0,2fr)_minmax(270px,.8fr)] xl:gap-16">
          <section aria-label={`${activeFilter} stories`}>
            <div className="divide-y divide-[#dcdde0] border-y border-[#dcdde0]">
              {stories.slice(0, visibleCount).map((story, index) => (
                <article className="group grid gap-5 py-6 sm:grid-cols-[210px_1fr]" key={story.title}>
                  <a className="aspect-[16/10] overflow-hidden bg-[#e8edf2]" href={`#/news/article-${index + 1}`} tabIndex="-1"><img className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.025]" src={story.image} alt={story.imageAlt} /></a>
                  <div className="flex min-w-0 flex-col py-0.5">
                    <p className="mb-2 text-[11px] font-semibold text-[#111318] uppercase">{story.category} <span className="font-normal text-[#5f6368]">· {story.date}</span></p>
                    <h2 className="m-0 text-[clamp(20px,2vw,27px)] leading-[1.15] font-semibold tracking-[-.025em] text-[#111318]"><a className="transition hover:opacity-60" href={`#/news/article-${index + 1}`}>{story.title}</a></h2>
          ...
          <aside className="lg:sticky lg:top-6 lg:self-start" aria-labelledby="news-most-read-title">
              ...
              <div className="divide-y divide-[#dcdde0]">
                {mostReadStories.map((story, index) => (
                  <article className="grid grid-cols-[1fr_88px] gap-4 py-4" key={story.title}>
                    <div>
                      <p className="mb-2 text-[10px] font-semibold text-[#5f6368] uppercase">{story.category} · {story.time}</p>
                      <h3 className="m-0 text-[15px] leading-[1.35] font-semibold text-[#111318]"><a className="transition hover:opacity-60" href={`#/news/popular-${index + 1}`}>{story.title}</a></h3>
                    </div>
                    <a className="aspect-square overflow-hidden bg-[#e8edf2]" href={`#/news/popular-${index + 1}`} tabIndex="-1"><img className="h-full w-full object-cover" src={story.image} alt={story.imageAlt} /></a>
                  </article>
                ))}
              </div>
            </div>
