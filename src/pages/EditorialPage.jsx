    ...
    <main className="bg-white px-3 py-10 md:px-6 md:py-14">
      <div className="mx-auto max-w-[1180px]">
        ...
        <article className="grid gap-8 border-b border-[#dcdde0] py-9 lg:grid-cols-[1fr_420px] lg:items-center">
          <div>
            <p className="mb-3 inline-block bg-[#60758a] px-2 py-1 text-[10px] font-bold tracking-[.08em] text-white uppercase">The editorial board</p>
            <h2 className="m-0 max-w-3xl font-serif text-[clamp(32px,4vw,52px)] leading-[1.05] tracking-[-.04em]"><a href="#/editorial/featured">{lead.title}</a></h2>
            <p className="my-5 max-w-2xl text-[15px] leading-7 text-[#4f5359]">{lead.summary}</p>
            <div className="flex items-center gap-5 text-[11px] font-medium text-[#5f6368]"><span>{lead.date}</span><a className="flex items-center gap-2 font-semibold text-[#111318]" href="#/editorial/featured">Read editorial <Icon name="arrow" /></a></div>
          </div>
          <a className="order-first aspect-[4/3] overflow-hidden bg-[#e8edf2] lg:order-none" href="#/editorial/featured"><img className="h-full w-full object-cover" src={lead.image} alt={lead.imageAlt} /></a>
        </article>
